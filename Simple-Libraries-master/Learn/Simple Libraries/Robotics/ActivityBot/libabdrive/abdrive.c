...
#define thetaMax 1078
#define thetaMin 32

static volatile int unitsFC = 64;
static volatile int angleM[2];
static volatile int thetaM[2];
static volatile int thetaR[2];
static volatile int thetaT[2];
static volatile int angleMd[2];
static volatile int anglePrev[2];
static volatile int thetaQ = (thetaMax - thetaMin) / 4;
static volatile int t[2], to[2];
volatile int tt[2];
static volatile int turnsR[2], turnsM[2], turnsTarget[2];


void abd_measureAngle()
{
  t[ABD_L] = pulse_in(14, 1);
  tt[ABD_L] = t[ABD_L];
  t[ABD_R] = pulse_in(15, 1);
  tt[ABD_R] = t[ABD_R];
  
  to[ABD_L] = t[ABD_L];
  to[ABD_R] = t[ABD_R];

  thetaM[ABD_L] = ((tt[ABD_L] - thetaMin) * unitsFC) / (thetaMax - thetaMin + 1);
  thetaM[ABD_L] = unitsFC - thetaM[ABD_L];
  thetaM[ABD_R] = ((tt[ABD_R] - thetaMin) * unitsFC) / (thetaMax - thetaMin + 1);
  thetaM[ABD_R] = unitsFC - thetaM[ABD_R];
  
  anglePrev[ABD_L] = thetaM[ABD_L];
  anglePrev[ABD_R] = thetaM[ABD_R];
  
  angleM[ABD_L] = thetaM[ABD_L];
  angleM[ABD_R] = thetaM[ABD_R];
  
  abd_ticks[ABD_L] = angleM[ABD_L];
  abd_ticks[ABD_R] = angleM[ABD_R];
  
  abd_dca[ABD_L] = abd_dc[ABD_L] * abd_dsr;
  abd_dca[ABD_R] = abd_dc[ABD_R] * abd_dsr;
  ...
  while(1)
  {
    for(int lr = ABD_L; lr <= ABD_R; lr++)
    {
      //turnsR[lr] = thetaT[lr] / unitsFC;
      //thetaR[lr] = thetaT[lr] % unitsFC;

      t[lr] = pulse_in(14 + lr, 1);
      tt[lr] = t[lr];

      thetaM[lr] = ((tt[lr] - thetaMin) * unitsFC) / (thetaMax - thetaMin + 1);
      thetaM[lr] = unitsFC - thetaM[lr];
          
      if( (t[lr] < (thetaMin + thetaQ)) && (to[lr] > (thetaMax - thetaQ)) )
      {
        turnsM[lr]--;
      }
      else if( (to[lr] < (thetaMin + thetaQ)) && (t[lr] > (thetaMax - thetaQ)) )
      {
        turnsM[lr]++;
      } 
      to[lr] = t[lr];
  
      angleM[lr] = (unitsFC * turnsM[lr]) + thetaM[lr];
      if(lr == 0)
        abd_ticks[lr] = angleM[lr];
      else
        abd_ticks[lr] = -angleM[lr];
    }
  }
